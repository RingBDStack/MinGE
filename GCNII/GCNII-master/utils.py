...
def load_citation(split_seed, dataset_str="cora"):
    """
    ...
    train_val = [i for i in range(len(y)+500)]
    np.random.shuffle(train_val)
    # print(train_val)
    idx_train = train_val[0:len(y)]
    idx_val = train_val[len(y): len(y)+500]
...
def load_citation_lp(args,dataset_str="cora"):
    """
    Load Citation Networks Datasets.
    """
    if dataset_str == 'airport':
        ...
        print(labels)
        idx_val, idx_test, idx_train = split_data(labels, val_prop, test_prop, seed=args.split_seed)
        labels = torch.LongTensor(labels)
        idx_train = torch.LongTensor(idx_train)
        idx_val = torch.LongTensor(idx_val)
        idx_test = torch.LongTensor(idx_test)
    ...
    adj = sparse_mx_to_torch_sparse_tensor(adj)
    lpdata = {}
    lpdata['adj_train'] = adj_train
    lpdata['train_edges'], lpdata['train_edges_false'] = train_edges, train_edges_false
    lpdata['val_edges'], lpdata['val_edges_false'] = val_edges, val_edges_false
    lpdata['test_edges'], lpdata['test_edges_false'] = test_edges, test_edges_false

    return adj, features, labels, idx_train, idx_val, idx_test,lpdata
...
def mask_edges(adj, val_prop, test_prop, seed):
    ...
    print("pos_edges shape",pos_edges.shape)
